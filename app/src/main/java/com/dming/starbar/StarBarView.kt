...
//        }   //评分星星
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        ...
        this.starDistance = mTypedArray.getDimension(R.styleable.RatingBar_starDistance, 0f).toInt()
        this.starSize = mTypedArray.getDimension(R.styleable.RatingBar_starSize, 20f).toInt()
    ...
    fun makeStarMark(mark: Float){
        if (integerMark) {
            starMark = Math.ceil(mark.toDouble()).toInt().toFloat()
        } else {
...
//                starMark = x * 1.0f / (measuredWidth * 1.0f / starCount)
                makeStarMark(x * 1.0f / (measuredWidth * 1.0f / starCount))
            }
